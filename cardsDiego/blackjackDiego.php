...
=======
     <h1>blackJack</h1>
>>>>>>> b9c6e39b4d3ccdd4438d137d5eab9ba2724142fd
<?php
    echo '<table><tr><td>';
    $deck = getdeck();
    array_pop($deck);
    ...
    shuffle($deck);
<<<<<<< HEAD
    $names=['Capi','Hulk','Thor','Widow','Spider','iron'];
...
<<<<<<< HEAD
    foreach($names as $name){
        for ($i=0;$i<2;$i++) {
            ...
            if($players[$name][$i]["value"]=='comodin'){
                if(){
                    $players[$name][$i]["value"]=11;
                }
                    else{$players[$name][$i]["value"]=11}
            }
            $points[$name]+=$players[$name][$i]["value"];
            if($players[$name][$i]["value"]>21){
                $interruptor=true;
            }
        }
    }
    ...
    foreach($points as $value){
        echo 'Estos son sus puntos jugador: '.$names[$i] .' ' .$value.'<br>';
    $i++;
    }
